...
(function(window){
	...
	Map.prototype.search = function(callback){
		...
		service.nearbySearch(request, function nearbySearch (result, status, pagination){
			if(status == google.maps.places.PlacesServiceStatus.OK){
				...
				result.forEach(function(r){
					let restaurant = {
						...
						id: r.place_id,
						avg: r.rating,
						user_total: r.user_ratings_total,
						ratings: [],
						scope: r.scope
					}
	...
	Map.prototype.getDetails = function(id, callback){
		...
		service.getDetails(request, function getDetails (result, status){
			if(status == google.maps.places.PlacesServiceStatus.OK){
				let reviews = [];
			...
			self.geocoder.geocode(
				...
				function(result, status){
					if(status == google.maps.GeocoderStatus.OK){
						...
						alert("Il n'y a rien par ici ! Zoomez pour plus de précision ou cliquez ailleurs !");
					}
				}
			);
		});
	...
	Map.prototype.render = function(command, parameter){
		switch(command){
			...
			case "addPopup":
				...
				break;
			case "updatePopup":
				this._updatePopup(parameter.id, parameter.content);
				break;
			case "cursor":
				this._cursor(parameter);
				break;
		}
	}
	...
	Map.prototype._updatePopup = function(id, content){
		const self = this;
		this.popups.find(function(popup){
			if(popup.id == id){
